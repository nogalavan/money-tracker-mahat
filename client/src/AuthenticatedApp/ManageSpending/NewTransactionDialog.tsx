...
const NewTransactionDialog = ({open, handleClose}: NewTransactionDialogProps) => {
  ...
  const handleSubmit = () => {
    ...
    addTransaction(amount, new Date().toLocaleString(), discription, catagory, type, user.id);
    handleClose();
  }
        ...
        <Stack direction='row' alignItems='center' justifyContent='space-between'>
          <FormControl fullWidth>
            ...
            <Select value={catagory} 
                    ...
                    sx={{m: '5px', width: '200px', color: 'black'}}>
              {selectOptions.map(x => <MenuItem value={x}>{x}</MenuItem>)}
            </Select>
          </FormControl>
          <Stack direction='row' alignItems='center'>
            <Typography>הוצאה</Typography>
            <GreenSwitch defaultChecked onChange={switchHandler}/>
